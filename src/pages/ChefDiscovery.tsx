...
import { 
  ChefHat, Star, MapPin, Clock, Search, Filter, 
  Sparkles, Award, Utensils, ArrowUpDown, X
} from 'lucide-react';
...
type SortOption = 'rating' | 'experience' | 'orders';
type FilterOption = 'all' | 'customizable' | 'veg' | 'non-veg';

export const ChefDiscovery = () => {
  ...
  const getFilteredAndSortedChefs = () => {
    let filtered = [...chefs];
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(chef => 
        ...
        chef.serviceArea?.toLowerCase().includes(query)
      );
    }

    // Category filter
    if (filterBy === 'customizable') {
      filtered = filtered.filter(chef => chef.dishes.some(d => d.allowsCustomization));
    } else if (filterBy === 'veg') {
      filtered = filtered.filter(chef => chef.dishes.some(d => d.category === 'veg'));
    } else if (filterBy === 'non-veg') {
      filtered = filtered.filter(chef => chef.dishes.some(d => d.category === 'non-veg'));
    }
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Browse our curated selection of talented home chefs. Each brings unique flavors and specialties to your table.
            </p>
          </div>
          ...
          <div className="mb-8 space-y-4">
            ...
            {/* Filter Options */}
            {showFilters && (
              <div className="p-4 rounded-xl bg-secondary/50 border animate-slide-up">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {[
                        ...
                        { value: 'veg', label: 'Veg Options' },
                        { value: 'non-veg', label: 'Non-Veg Options' },
                      ].map((option) => (
                        <Button
                          key={option.value}
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="font-display text-xl font-bold mb-2">No chefs found</h3>
              <p className="text-muted-foreground">Try adjusting your search or filters</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-6">
              {filteredChefs.map((chef) => {
                const specialDish = getSpecialDish(chef.dishes);
                const hasCustomizable = chef.dishes.some(d => d.allowsCustomization);
                
                return (
                  <Card 
                    key={chef.id} 
                    className="shadow-card hover:shadow-card-hover transition-all cursor-pointer group overflow-hidden"
                      ...
                      <div className="p-6">
                        <div className="flex gap-4">
                          ...
                          <div className="relative">
                            <div className="w-20 h-20 rounded-xl bg-chef flex items-center justify-center group-hover:scale-105 transition-transform">
                              <ChefHat className="w-10 h-10 text-chef-foreground" />
                            </div>
                            {(chef.avgRating || 0) >= 4.5 && (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-warning/10">
                                ...
                                <span className="text-sm font-medium">{chef.avgRating?.toFixed(1) || 'New'}</span>
                              </div>
                              <span className="text-xs text-muted-foreground">
                                ({chef.reviewCount || 0} reviews)
                              </span>
                              {chef.totalOrders && chef.totalOrders > 500 && (
                            ...
                            <div className="flex items-center gap-1 text-xs text-muted-foreground">
                              ...
                              <span className="truncate">{chef.serviceArea}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-4">
                          ...
                          {chef.deliverySlots?.includes('lunch') && chef.deliverySlots?.includes('dinner') && (
                            <Badge variant="outline" className="text-xs">
                        ...
                        <div className="px-6 pb-6">
                          <div className="p-3 rounded-lg bg-primary/5 border border-primary/10">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {specialDish.nutritionalInfo.calories} cal • {specialDish.nutritionalInfo.protein}g protein
                            </p>
                          </div>
                        </div>
                      )}
